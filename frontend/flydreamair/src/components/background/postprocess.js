...
class PostProcessPass {
	constructor(scene) {
		...
		this.scene_texture_plane = new THREE.WebGLRenderTarget(window.innerWidth, window.innerHeight, {
			format: THREE.RGBAFormat,
			type: THREE.FloatType,
			depthBuffer: true,
			magFilter: THREE.LinearFilter,
		...
		this.bright_filter_texture = new THREE.WebGLRenderTarget(window.innerWidth/2, window.innerHeight/2, {
			format: THREE.RGBAFormat,
			type: THREE.FloatType,
			depthBuffer: true,
			magFilter: THREE.LinearFilter,
		...
		this.hblur_texture = new THREE.WebGLRenderTarget(window.innerWidth/5, window.innerHeight/5, {
			format: THREE.RGBAFormat,
			type: THREE.FloatType,
			depthBuffer: true,
			magFilter: THREE.LinearFilter,
		...
		this.vblur_texture = new THREE.WebGLRenderTarget(window.innerWidth/5, window.innerHeight/5, {
			format: THREE.RGBAFormat,
			type: THREE.FloatType,
			depthBuffer: true,
			magFilter: THREE.LinearFilter,
	...
	render_hblur(renderer, camera) {
		shaders.horizontal_blur.uniforms.target_height.value = this.hblur_texture.width;
		shaders.horizontal_blur.uniforms.scene_texture.value = this.bright_filter_texture.texture;
	...
	render_vblur(renderer, camera) {
		shaders.vertical_blur.uniforms.target_height.value = this.vblur_texture.height;
		shaders.vertical_blur.uniforms.scene_texture.value = this.hblur_texture.texture;
