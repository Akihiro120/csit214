...
import { FlightSearchResult, SearchResult } from "../../../components/SearchResult";
...
function Search() {
	...
	const submitRequest = async (flightId: string) => {
		try {
			const postResponse = await apiClient.post("/api/booking/search", { flight_id: flightId });
	...
	return (
		<div className="flex flex-col gap-4 bg-white" id="result">
			...
			{data.flights.map((flight: FlightSearchResult) => (
				<SearchResult key={flight.flight_id} flight={flight} onclick={() => submitRequest(flight.flight_id)} />
			))}
		</div>
	);
}
