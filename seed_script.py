...
for code in airport_codes:
    airports.append((code, airport_cities.get(code) + " Airport", airport_cities.get(code), "Australia", "AEST"))
cursor.executemany("""
...
for i in range(0, len(airport_codes)):
    for j in range(0, len(airport_codes)):
        if i != j:
            routes.append((airport_codes[i], airport_codes[j]))
cursor.executemany("""
    INSERT INTO route (origin_airport_code, destination_airport_code)
    VALUES ( %s, %s)
""", routes)
...
for route_id in route_ids:
    # For each route, assign a unique flight number for each base departure hour.
...
while current_date <= end_date:
    for route_id in route_ids:
        for base_hour in base_hours:
            # Create a base datetime for the flight at the scheduled hour
            base_dep = datetime.combine(current_date, datetime.min.time()).replace(hour=base_hour)
            # Add a small random offset (±15 minutes) so departures are not exactly on the hour
            ...
            arr_time_dt = dep_time_dt + timedelta(hours=flight_duration)
            arr_time = arr_time_dt.time()
            # Use the pre-assigned unique flight number for the route and base_hour
            flight_number = flight_numbers[(route_id, base_hour)]
            flights.append((flight_number, route_id, current_date, dep_time, arr_time))
    current_date += timedelta(days=1)

cursor.executemany("""
    INSERT INTO flights (flight_number, route_id, flight_date, departure_time, arrival_time)
    VALUES (%s, %s, %s, %s, %s)
""", flights)
