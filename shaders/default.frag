...
void main() {
	...
	vec3 lo = vec3(0.0f);
	{
		...
		float NdotL = max(dot(N, L), 0.0);
		lo += (kD * albedo / PI + specular) * vec3(5.0) * NdotL;
	}

	vec3 kS = fresnel_schlick(max(dot(N, V), 0.0), F0);
	vec3 kD = 1.0 - kS;
	kD *= 1.0 - metallic;
	vec3 irradiance = texture(irradiance_map, N).rgb * vec3(2.0f);
	vec3 diffuse = irradiance * albedo;
	vec3 ambient = kD * diffuse;
	vec3 R = reflect(-V, N);
	vec3 prefilteredColor = textureLod(cube_map, R, roughness).rgb;
	float shadow = shadow_calculation(N, L);
	vec3 color = ambient + (lo + prefilteredColor) * (1.0 - shadow);
